...
// Two-stage warp-shuffle max reduction.
// BLOCK_SIZE must be a multiple of 32.
// Stage 1: each warp reduces 32 thread-local values to one warp max via
//          __shfl_down_sync, lane 0 writes to shared memory.
// Stage 2: the first warp reduces the WARPS_PER_BLOCK warp maxima to one
//          block max via another round of __shfl_down_sync, lane 0 writes
//          one value per block to global memory.
...
__global__ void compute_block_max_speed_kernel(
    ...
    // -------------------------------------------------------------------------
    // 1. Each thread computes its local max wave speed.
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // 2. Warp-level reduce via shuffle down (5 steps for 32 lanes).
    //    Out-of-range threads contribute 0.0, safe for max.
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // 3. Lane 0 of each warp writes its warp max to shared memory.
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // 4. First warp reduces all warp maxima to a single block max.
    //    Lanes beyond WARPS_PER_BLOCK load 0.0, safe for max.
    // -------------------------------------------------------------------------
    if (warpId == 0) {
