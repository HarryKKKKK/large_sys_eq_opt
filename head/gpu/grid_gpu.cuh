...
public:
    ...
    Grid2DGPU(Grid2DGPU&& other) noexcept {
    ...
    void upload_from_aos(const std::vector<Conserved>& host_data) {
        std::vector<double> rho_h(num_cells());
        std::vector<double> rhou_h(num_cells());
        std::vector<double> rhov_h(num_cells());
        std::vector<double> E_h(num_cells());
...
private:
    ...
    void move_from(Grid2DGPU&& other) {
        ...
        ny__ = other.ny__;
        ng__ = other.ng__;
        x_min__ = other.x_min__;
        x_max__ = other.x_max__;
        y_min__ = other.y_min__;
        y_max__ = other.y_max__;
        dx__ = other.dx__;
        dy__ = other.dy__;
...
struct Grid2DGPUView {
    ...
    __host__ __device__
    int total_nx() const { return nx + 2 * ng; }

    __host__ __device__
    int total_ny() const { return ny + 2 * ng; }
...
inline Grid2DGPUView make_view(const Grid2DGPU& grid) {
    return Grid2DGPUView{
        grid.nx(), grid.ny(), grid.ng(),
        grid.x_min(), grid.x_max(), grid.y_min(), grid.y_max(),
        grid.dx(), grid.dy(),
        const_cast<double*>(grid.rho_ptr()),
        const_cast<double*>(grid.rhou_ptr()),
        const_cast<double*>(grid.rhov_ptr()),
        const_cast<double*>(grid.E_ptr())
    };
}